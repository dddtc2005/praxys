...
- **Registration boundary** (one explicit carve-out from full parity). The miniapp owns *exactly one* registration-adjacent write: **`POST /api/auth/waitlist`** for private-alpha intent capture, since WeChat is the discovery channel for the CN audience and forcing them out to a browser is a friction tax. Everything else in the registration / account-creation path — invitation-code redemption, email + password account creation, the platform-connection wizard for Garmin / Stryd / Oura — stays on praxys.run because OAuth-in-WebView is painful. If a future write is *also* "intent capture without setup," it's a candidate to lift here; if it touches credentials or platform connections, default to "praxys.run only."
