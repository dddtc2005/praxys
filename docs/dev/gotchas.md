...
**1. DI Bearer exchange is pinned to `diauth.garmin.com`.** That host has no record of CN accounts — all four `DI_CLIENT_IDS` return 400/401. Without a DI token the library falls back to the JWT_WEB cookie, which is accepted by `connect.garmin.cn/modern/` (HTML) but *not* by `connectapi.garmin.cn/<any-service>` — the API gateway responds `{"message":"HTTP 403 Forbidden","error":"ForbiddenException"}` regardless of browser UA, cookie jar, or warmup GETs. Garmin runs a parallel `diauth.garmin.cn` that accepts the same `GARMIN_CONNECT_MOBILE_ANDROID_DI_2025Q2` client IDs and the same hardcoded `grant_type` identifier (verified via `scripts/garmin_cn_api_probe3.py`), so we override `Client._exchange_service_ticket` on CN instances to temporarily swap `DI_TOKEN_URL` to `diauth.garmin.cn` for the duration of the call. The swap is instance-scoped so concurrent international logins aren't affected.
